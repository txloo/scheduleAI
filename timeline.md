...
2026-09-18 ✅ OpenRouter LLM migration — replaced OpenCode Zen provider with OpenRouter (model z-ai/glm-5.2:free) via new functions/openrouter.js; LLM_PROVIDERS dispatcher (callLLM) so future providers are drop-in modules; /model-<model-id> chat command switches model per user without history (active model + openrouter/ label shown in ChatPanel header)
2026-09-19 ✅ Text tool protocol (client-side toggle) + 429 retry + error weeding — web sends toolProtocol per message (Native/Text pill), Telegram /tool persists it in the chat doc; OpenRouter 429 retried up to OPENROUTER_MAX_RETRIES (Retry-After/2s backoff); /error-prefixed bubbles kept visible in RTDB history but excluded (with their paired user message) from the LLM context for web+Telegram; unused /mode command removed from web, Telegram keeps it + /tool
2026-09-19 ✅ Planner-mode enforcement + mode notice — "Now In *Planner|Builder* mode" prepended to the latest user message sent to the LLM only (never persisted); blocked Planner write tools shown as 🔧 name: ⛔ blocked (Planner mode) bubbles in history and reported back to the model in one [Planner mode] feedback message so it self-corrects both web and Telegram (all-blocked batch still runs a correction round via the follow-up loop gate)
